...
**Agent workflow (MANDATORY on every push):**
1. Get latest tag: `git tag --sort=-v:refname | head -1`
2. Parse version, increment YY (patch)
3. After `git push`: run `git tag vX.XX.YY && git push origin vX.XX.YY`
4. Report version in push confirmation

**On user commands:**
- "bump minor": increment XX, reset YY to 00, then push+tag
- "bump major": increment X, reset XX and YY to 00, then push+tag

**First push on new project:** Start at `v0.00.00`, first push becomes `v0.00.01`
