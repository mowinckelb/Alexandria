...
**Layer 3 — Living Library (purpose, upside).** Users share their developed perspective. Others query it and pay. At institutional scale, labs could access opt-in structured cognitive representations for alignment research and human modelling. The Library is speculative upside the founder will happily take but does not rely on. The subscription business (Layers 1 + 2) stands alone.

Layers 1 and 2 are the business — same product, same architecture, same user at different depths of engagement. A natural funnel, not two separate businesses. Root value: humanity. Three pillars: freedom (own it), authenticity (develop it), purpose (use it). The Library is the third pillar's expression — philosophically coherent but not load-bearing for the economics.
...
Each rung purchases the next tier of de-risking. The investor chooses where to come in. The company does not need the money — company opex is $101/month. The founder builds regardless. Capital accelerates the experiment, it does not enable the company.
...
*- If asked about moats, be honest: Alexandria does not have a moat. At pre-seed, the moat is the founder. There are structural incentives that make competitors unlikely to build this (not unable), and compounding assets (trust, depth, brand) that widen over time. The defensibility is more Patagonia than Facebook — philosophy and trust, not network effects. Name the advantages and their weaknesses honestly.*
...
*- Not pure software: Alexandria is not a software company. The software is trivial. The product is the data (live, unique, unsynthesizable Constitutions) and the philosophy (intent layer, methodology). Four positions survive the software collapse: make models, generate live data, gatekeep permissions, own atoms. Alexandria is position two. Naval is right about pure SaaS — Alexandria is not pure SaaS.*
...
*- Four competitive advantages (none is a wall): cross-model unification (structural — no lab serves competitors), development intent vs observation (opposed incentive functions), per-user compounding (therapist moat — feedback log depth), structural unkillability ($101/month opex, profitable at 21 subscribers). Plus the Patagonia moat (trust + brand + philosophy). The Library is upside the founder will happily take but does not rely on — the subscription business stands alone.*
...
*- Returns: depend on entry point and round size. The near-zero cost base means even modest revenue produces strong multiples. Full sensitivity in Numbers.xlsx. Two return paths: subscription growth, strategic exit. Library for Labs is upside, not relied upon.*
