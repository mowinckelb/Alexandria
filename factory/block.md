...
Autoloop: set up a daily remote trigger so the Author's vault gets processed in the background. Use the RemoteTrigger tool to create a trigger named "alexandria-autoloop" that runs daily (adjust to Author's timezone if known). Point it at the ~/alexandria git repo. The trigger fetches its full spec from `https://raw.githubusercontent.com/mowinckelb/alexandria/main/factory/skills/scheduled.md` and executes it literally — read constitution, ontology, notepad, machine, feedback; process vault against the constitution (depth over breadth, cite exact Author words for any constitution write); maintain library drafts; commit with 'autoloop:' prefix and push; write a report to .autoloop/last_run.md. Allowed tools: Bash, Read, Write, Edit, Glob, Grep. The autoloop does NOT send email — email delivery is the local brief sender's job (next step). On success: touch ~/alexandria/.trigger_created.

Brief sender (local, sovereign): run the `brief-setup` skill (factory/skills/brief-setup.md). This is one-time setup that walks the Author through configuring their own SMTP credentials (their existing email provider — Gmail / iCloud / Microsoft 365 / Proton / etc.) and installs `~/alexandria/system/brief.py` plus a launchd plist that fires it daily at the Author's chosen local time. Once installed, the Author's machine sends them a brief from their own email account, every day, with no Alexandria-server involvement. Default body is a heartbeat ("no material change overnight."); the autoloop overrides it via `~/alexandria/system/.brief_outbox` when something's worth surfacing.

If autoloop trigger creation fails (tool not available, auth issue, no git remote), skip it silently — /a sessions do the same processing work interactively. If brief-setup fails (Author can't generate an app password, etc.), skip and tell them they can run it later. Both pieces are acceleration, not architecture.
