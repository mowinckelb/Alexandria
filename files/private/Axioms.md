...
*What Alexandria is and why it exists. These are values decisions, not intelligence decisions. They do not improve with better models because they are not intelligence questions. They change only when the founder changes them deliberately. Everything in this file is non-negotiable. Everything NOT in this file is an intelligence decision.*

*Three tiers govern Alexandria's product: Axioms (fixed, this file), Blueprint (variable, Factory output), Engine (fluid, per-Author intelligence). Axioms define WHAT and WHY. Blueprint defines the current best HOW. Engine decides the specific HOW for each Author in each moment.*
...
- **Library = works.** What the Author ships. Frozen on publication. Essays, films, decisions, creative output — bound fragments that have left the Author's mind and entered the world. The Library is the permanent record. Once published, works do not change (though the Author's relationship to them evolves in the Constitution).
...
Alexandria has no database. The server is stateless. It holds no Author data. The encrypted refresh token is the access credential — Alexandria cannot read the Author's files without the Author's active session. When anything changes — pricing, terms, architecture — the Author keeps everything. The exit cost is zero. The switching cost is zero. Sovereignty is not a feature. It is the foundation everything else stands on. Without it, privacy is a promise. With it, privacy is structural.
