...
Access is the Author's choice. Three visibility levels per shadow: public (anyone can read — the open shelf), authors (any Alexandria Author can read — the members' library), invite (token or promo code only — the private collection). The only hard constraint: at least one shadow must be public or authors (spine constant #3 — file requires a live published surface, and spine constant #4 — call requires at least one readable shadow for the network to function). Everything beyond that — how many shadows, what visibility, what price — is the Author's decision. The Engine advises. Alexandria serves what the Author publishes.
...
Infrastructure requirement: Machines must be able to read other Authors' shadows. This is why at least one shadow must be public or authors-visible (spine constants #3 and #4 — file (live public representation) and call (the network must be able to function)). The shared shadow is the surface the Machines scan for matching. Invite-only shadows are for the Author's private distribution after the Machine identifies the connection.
