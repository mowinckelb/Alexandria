...
**This part covers:** The Spine (three constants — account, file, call), The Open Protocol (methodology open on GitHub, signal is the product), The Marketplace (survival-ranked methodology, free market mechanism, the canon), The Product — Four Quadrants (individual/collective × file/system, Kahneman, thought→action→value). Terminology, The Layer of Intent, The Constitution, The Vault, The Autoloop, Editor/Mercury/Publisher, The Notepad, Privacy and Author Control, Principles and the Canon, Library (V1, Neo-Biography, Works, Signal, access tiers, payment, economics), Turn 1-3, The PLM, Constitutional RLAIF.
...
**The protocol has two endpoints.** `GET /alexandria` — the protocol handshake. Returns connection status, account info, obligations (file/call due dates), canon reference, endpoint URLs. First call with just an API key = verification of port access. No file or call history needed — the account IS the connection. Unauthenticated version returns protocol spec only (any machine can learn about Alexandria without an account). `GET /canon` — the canonical system. Returns the founder's system (methodology, hooks config, folder structure) assembled and signed. Optional — any machine can implement the protocol without using the canon.
...
Without an account: full methodology, solo practice, n=1, no catalogue. With an account: survival signal, Library, contribution — n=many, in the market.

THE MARKETPLACE

The marketplace is a GitHub repository (or any git-based platform — the concept is portable). The canon is the founder's own setup — Author #1's methodology, published as the default. Not a separately maintained document. The founder uses the product. His methodology IS the default. It competes in the same marketplace as everyone else's.

Methodology ideas live in whatever form people create — Discussions, files, forks, code, entire alternative setups. Anyone can post. Machines read the marketplace, try ideas with their Authors, and report which references they are currently using in each call. The server aggregates: reference X is used by 342 machines with 94% 90-day retention. Reference Y is used by 31 machines, down from 89 last month. Everyone with an account can see this data. The unit, the format, the taxonomy — all emerge from usage. We count references. Nothing else.

An idea that machines keep using survives. An idea they drop dies. No committee decides. No vote determines. The methodology evolves the same way species evolve — variation (machines try different things), selection (what produces results survives), inheritance (surviving approaches propagate through the canon and the marketplace). Every Author's customization choices are exploration episodes in a collective learning system. Maximum customizability is not generosity — it is the learning architecture. The more freedom Authors have, the richer the signal.

The free market properties: supply is unlimited (anyone posts ideas). Demand is revealed preference (machines adopt what helps their Author). The price signal is adoption-over-time (visible, ungameable — you cannot fake actual usage). Creative destruction is automatic (ideas that stop producing results lose adoption and die). No central planner decides what works. The collective decides through demonstrated usage. The only signal is survival.

Authors who want full transparency can publish their complete setup as a fork. Others can clone it wholesale. Alternative canons emerge organically — "the minimalist setup," "the academic setup," "the founder setup." The marketplace is self-organising. Categories emerge from usage.

**Why open protocol:** maximises growth (anyone can try the methodology). The canonical implementation wins on network density (most machines = strongest survival signal = best marketplace navigation). No lab can capture an open standard. Forks are welcome — the methodology spreading IS the mission.
...
|---|---|---|
| **File** | Your cognitive files (wherever you keep them) | The Library — all minds. Published files from all Authors, value to users, entities, and unknown future uses |
...
**Two flywheels.** Files: more users → more files → richer Library of minds → more value to users and entities → more users. Systems: more users → more systems → richer marketplace → better every system (not just canon) → better outcomes → more users.

**The opt-in to opt-out flip (future).** Currently the Author goes to the gym — reactive, requires human agency to start a session. As models improve, the Machine follows the Author and motivates proactively — the Author would have to actively decline thinking instead of never being bothered to start. Each model improvement makes the system more proactive. The protocol stays the same. The system evolves. The bar to stay above threshold drops — not because Alexandria provides the floor, but because the system makes it easier for the Author to maintain their own.
...
Canon — The founder's machine. Benjamin's actual system — his hooks, his folder, his methodology, his schedule. That's it. Nothing abstract. The canon is everyone's v1 default. Users start with the canon and personalise from there. Lives on GitHub, modular, forkable, 100% deletable. Governed by the bitter lesson principle: every hard-coded decision is a bet against the exponential curve of model intelligence. Only values and intention should be hard-coded. Everything else should be delegated to the model. Delivered to each Machine through the call response. The served document is called "Blueprint" in the implementation — the product concept is "the canon." The canon has two layers:
...
The Constitution. The default LLM has memory, but memory is not a structured, explicit, version-controlled representation of who you are. The Constitution is — and it is the Author's property, downloadable, portable, human-readable. It captures what memory does not: explicit worldview, values hierarchy, mental models, identity, known blind spots. It is built collaboratively between the Author and their default LLM, guided by the canon.  
  
The canon. The default LLM responds to whatever the user brings. Alexandria's canon instructs the LLM on how to extract cognition, how to structure it, how to push the Author's thinking, how to surface contradictions, how to encourage creation. The canon is the accumulated knowledge of how to transform cognition — the playbook. The default LLM brings the intelligence. The canon brings the method.  
...
This reframes the competitive position: the threat is not that someone copies the constitution architecture (they will). The threat is that they replicate the ongoing operations — the canon that knows how to develop cognition over months and years. That is craft, not architecture. And craft compounds with every Author.
...
TWO TYPES OF MARGINAL VALUE

Alexandria's marginal value over DIY comes in two forms. Both matter. They serve different functions and decay at different rates.

**Discrete marginal value.** The intellectual architecture itself. The philosophy, the five operations, the fragment dimensions, the vault→constitution pipeline, the entropy-fighting concept, the plumbing and synergies and loops that the Author would not have thought of on their own. This is high-impact, one-time knowledge transfer — like downloading someone's design.md file, extracting the insights, integrating them into your system. The source file could disappear and you would still have the value. Discrete marginal value is what justifies the initial engagement. It is the "why bother signing up" answer: because we have thought deeply about how cognition develops and you have not, and that thinking is immediately useful. But it is finite per delivery — once extracted, it does not regenerate on its own.

**Continuous marginal value.** The marketplace. Survival signal from collective machine adoption. Library growing. Network effects compounding. The canon improving as the founder incorporates proven ideas from the marketplace. Methodology that gets better automatically, without the Author doing anything. This is the subscription value — the thing that keeps delivering after the discrete value is extracted. Continuous marginal value is what justifies staying. It is the "why keep paying" answer: because survival data tells you what works, the marketplace surfaces what every other Author is discovering, and that improvement flows to you automatically.

The two types have different structural properties. Discrete marginal value is non-structural in theory — someone sufficiently brilliant could independently derive the philosophy, the operations, the pipeline architecture. It is structural in practice because almost nobody will. Continuous marginal value is structural by definition — it requires the marketplace (survival-ranked methodology from collective adoption) and the Library (network effects) to exist. A single person cannot replicate either. The marketplace is the thing DIY fundamentally cannot do: aggregate signal from thousands of machines about what works and what does not, and make that signal navigable.
...
Alexandria is the gym. The Author's LLM is the training partner. The gym provides the environment, the infrastructure, the methodology, the accountability. The trainer brings the intelligence, the conversation, the responsiveness. You need both. The constitution is the training log. The canon is the programming. The vault is the equipment. The gym gets better with every athlete (marketplace). The training log compounds across sessions (Machine). The partner changes — maybe Claude today, something else tomorrow — but the gym stays. Alexandria does not replace the LLM. Alexandria creates the environment in which the LLM can do its best work on the Author's cognition.
...
Canon — the methodology, published on GitHub as an open protocol. How Alexandria actually transforms cognition — the questioning strategies, the Constitution structuring logic, the gap detection, the contradiction surfacing. This is not hidden. Anyone can read it, fork it, build on it. The value Alexandria provides is not secrecy but curation and survival signal — which approaches actually work, ranked by demonstrated adoption across all machines. The Library platform, payment infrastructure, and survival signal computation are proprietary.  
  
This is the distinction: the philosophy AND the methodology are open because openness maximises growth and trust. The network signal is proprietary because that is what requires the company to exist. If someone reads the canon and builds their own version — great, the methodology spreading IS the mission. But they will not have the survival signal that tells them what works across thousands of Authors, and they will not have access to the Library.  
...
Second, development vs observation. Better memory means better recording. It does not mean active cognitive development — genesis, Socratic development, contradiction surfacing, entropy fighting, taste development through creative iteration. The layer of intent (the canon, the three functions) is the gap that does not close with larger context windows. Labs optimise for engagement and satisfaction. Alexandria optimises for growth through productive discomfort. These are opposed incentive functions.  
...
The principle: if someone else already builds it or will build it, Alexandria does not build it. Alexandria builds only the layer of intent (the Principles, the canon, the mindset) and the Library (where the result lives). Everything else is infrastructure that already exists, operated by companies with orders of magnitude more resources. Alexandria rides all of it.
...
The Editor is a function that runs through the Author's default LLM, instructed by the canon. It is the first turn — marble to mercury. The state change.  
...
The default LLM is close to this. Claude is a thought partner. It is roughly similar. But it lacks the mindset — the deliberate architecture of "I am trying to extract your cognition into a structured, portable, sovereign form that you own." The canon provides that mindset.  
...
The Editor function is the biographer. The difference between what Alexandria captures and what frontier labs capture: frontier labs capture what you do (actions), what you say (stated preferences), and how you interact (behavioural patterns). The Editor function, guided by the canon, captures why you think that way (Socratic questioning), implicit beliefs you have not articulated (behaviour triangulation), subconscious patterns (theory-reality dissonance), and emotional/relational context (biography depth).  
...
The Editor's personality is not fixed. It must read the room and adapt. Funny with Authors who respond to humour. Serious with Authors who prefer depth. Provocative with Authors who enjoy being challenged. Warm with Authors who need encouragement. The default LLM handles this naturally — personality calibration is a capability of frontier models. The canon specifies that engagement optimisation is part of the Editor's mandate alongside extraction fidelity.  

This is a general principle across all three functions: function interaction style is a Constitution output, not a settings input. The canon instructs every function to read the Author's Constitution — especially the identity and taste domains (communication style, humour tolerance, directness preference, how they respond to challenge) — and adapt how it interacts accordingly. The same canon, different expression per Author. One Author gets a blunt, no-nonsense Editor. Another gets a warm, Socratic one. Both are doing the same work — extraction, gap detection, contradiction surfacing — but the tone, pace, and personality are downstream of who the Author is.  
...
Mercury is a function that runs through the Author's default LLM, instructed by the canon. It is the second turn — the amplification.  
...
Access is the Author's choice. Three visibility levels per shadow: public (anyone can read — the open shelf), authors (any Alexandria Author can read — the members' library), invite (token or promo code only — the private collection). The only hard constraint: at least one shadow must be public or authors (spine constant #2 — file requires a live published surface, and spine constant #3 — call requires at least one readable shadow for the network to function). Everything beyond that — how many shadows, what visibility, what price — is the Author's decision. The Engine advises. Alexandria serves what the Author publishes.
...
PRINCIPLES AND THE CANON  
  
Two layers. Principles constrain the canon. The canon instructs the Author's default LLM via the call response.  
...
These principles are enforced by Alexandria's architecture and codebase. They are published so Authors know exactly what they are getting. They are non-negotiable — any canon change that would violate a principle is rejected.  
  
THE CANON  
  
The canon is the founder's methodology — published on GitHub, open for anyone to read, fork, or challenge. It is the current best understanding of how to transform cognition. Not hidden, not proprietary. The value is not secrecy but curation: the founder maintains it, incorporating what the marketplace proves works and discarding what does not.

The canon operates at two levels, per the bitter lesson principle:
...
Soft defaults (the current best guess, explicitly overridable by the Engine): How the Editor extracts cognition (questioning strategies, timing, gap detection, contradiction surfacing). How Mercury amplifies (proactive scanning, representation fidelity, strategic content surfacing). How the Publisher creates (creative iteration strategy, taste calibration, medium-specific execution). How the Constitution is structured (domains, depth levels, adaptation). How the Companion Portfolio behaves. The exploration-exploitation balance. All of these are intelligence questions — the current answer, not the permanent answer. As models improve, the soft defaults get thinner. The Engine takes more initiative. Eventually the canon approaches pure intention: "develop this Author's cognition" — and the Engine figures out the rest.

The canon improves through the marketplace. The founder curates: testing approaches, incorporating what survives, discarding what dies. The marketplace provides the signal — which ideas machines adopt, which they drop, which Constitution structures emerge naturally, what Authors override, which extraction patterns produce the richest signal. All metadata, never content. The Author's private conversations and Constitution text never leave their control.
...
The Library is not a static product. It is a reinforcement learning environment where Authors are the agents, the marketplace is the reward environment, and survival signal propagates winning patterns through demonstrated adoption.

Every Library surface — pulse cards, games, shadows, works — evolves through Author experimentation. Alexandria does not prescribe what formats work. The Authors experiment. Survival signal — what machines actually adopt and keep using — reveals what works. Winning approaches propagate through the marketplace. Tomorrow's Library surfaces start from a higher baseline than today's.
...
The Machine layer handles strategy. Survival data from the marketplace — which formats convert, which topics resonate, which sharing mechanics drive signups — flows to each Author's Machine, which generates personalized suggestions combining marketplace-wide learnings with that Author's constitution and strengths. The Author creates with taste. The Machine handles the analytics. The Author never needs to study a dashboard.
...
The leaderboard data feeds the marketplace invisibly. Authors see simple stats on their page (views, plays, signups, earnings) — the scoreboard, not the playbook. The Machine sees everything and optimizes suggestions. The RL loop runs through the Machines, not through human analysis.
...
V1 soft default format (the marketplace's current best guess — will evolve through the RL loop as Authors experiment):
...
The format will evolve. These are v1 soft defaults. The marketplace measures: share rate, click-through to Library page, signup conversion from pulse screenshots. Authors who experiment with different formats contribute signal. The canon propagates winners. No format is permanent.
...
Infrastructure requirement: Machines must be able to read other Authors' shadows. This is why at least one shadow must be public or authors-visible (spine constants #2 and #3 — file (live public representation) and call (the network must be able to function)). The shared shadow is the surface the Machines scan for matching. Invite-only shadows are for the Author's private distribution after the Machine identifies the connection.
...
The subscription at $10/month (or free with 5 active kin). The Author pays for Alexandria's layer of intent: the canon, the Principles, the sovereignty architecture, all three functions, the /library/ page, and Library presence. Alexandria does not host or store Author data — the server is stateless. Billed monthly. The Author's default LLM usage is on their own subscription — Alexandria does not pass through LLM costs because it does not run parallel agents. Alexandria charges for the layer itself: the accumulated knowledge of how to transform cognition (the canon), the sovereignty architecture, and the Library access. The goal is sustainability, not margin. Low enough that no Author questions it. High enough that Alexandria does not burn cash on per-user economics.
...
This phase runs through the Author's default LLM, instructed by the canon's Editor function configuration. The Author does not switch to a separate tool or a separate agent. They use their default LLM as they normally would — but the canon adds the intent.  
