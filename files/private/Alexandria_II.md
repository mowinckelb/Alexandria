...
4. Give Alexandria access to that file, with at least one file free to other Authors. This is the minimum that makes the network function — Authors can see each other, the Engine can match, the Library has content. Everything beyond that is the Author's choice: public access, paid tiers, enterprise deals, constraints on who sees what. But at least one file is visible to the network. A book that can never be opened is not a book in a library.
