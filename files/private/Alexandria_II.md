...
Factory — Alexandria's internal execution loop (Loop 1). Historically the mechanism for maintaining the methodology and aggregating cross-Author signal. Now evolving into the marketplace (see THE MARKETPLACE): a GitHub-based free market where methodology ideas compete on survival, ranked by collective machine adoption-over-time. The Factory's legacy role (daily trigger, dashboard, learnings file) is the seed implementation of what becomes the marketplace. The canon (the founder's methodology) replaces the centrally-curated Blueprint. The survival signal from all machines' calls replaces the founder's manual curation as the primary selection mechanism. See 'Feedback Loops' section for full architecture.  
...
Canon (served as the Blueprint) — The Machine's living design, governed by the bitter lesson principle: every hard-coded decision is a bet against the exponential curve of model intelligence. Only values and intention should be hard-coded. Everything else should be delegated to the model. The canon is the founder's methodology, open on GitHub, delivered to each Machine through the call response. The served document is called "Blueprint" in the implementation — the product concept is "the canon." The canon has two layers:
