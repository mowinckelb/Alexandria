...
Claude-first positioning — Alexandria is optimised for Claude users, and Claude is the lowest-friction MCP platform. Claude is the only platform where any user — including free tier — can add a custom MCP connector in under a minute. ChatGPT Plus ($20/mo) supports custom MCP connectors in Developer Mode — the Alexandria server works without code changes, same transport, same OAuth. Gemini requires an enterprise allowlist. The MCP server is platform-agnostic by design: any MCP-compatible client can connect. The onboarding is three steps: add the connector (Settings → Connectors → add URL → authorise cloud storage), paste two memory edits into the LLM's account memory, and say "alexandria." Every Claude user is a potential Sovereignty Author on day one, and every ChatGPT Plus user is reachable with a slightly more technical onboarding. The layer of intent integrates most deeply with Claude's affordances (Projects for Constitution context, MCP for data connections). On other platforms, the layer may be more manual — Constitution files pasted into context windows rather than integrated via Projects. But the sovereign data is always portable. The Claude-first positioning is a go-to-market choice, not an architectural constraint: the early adopters are the Claude power users, and the protocol works everywhere MCP works.
...
Sovereignty has no natural cancellation point. The product works across every AI the user touches. As long as the user uses AI — which is increasingly synonymous with using a computer at all — the layer adds value. The Constitution compounds with every conversation. Cancelling means abandoning a compounding cognitive asset. The only structural reasons to cancel: the user stops using AI entirely (increasingly impossible), finds a strictly superior alternative (the therapist moat makes switching costly — the accumulated feedback log is calibrated to this specific person), or does not value self-knowledge at all (a self-selecting minority who would not subscribe in the first place). For every other user, Sovereignty is a permanent subscription. The kin mechanic reinforces this — cancelling raises your kin's prices, creating social pressure against churn. The financial model uses 4% monthly churn as a conservative assumption for a new product. The structural expectation is that churn trends toward near-zero as Constitutions deepen. This has direct implications for LTV calculations: if average retention is measured in years rather than months, the lifetime value of a $5/month subscriber is an order of magnitude higher than standard SaaS assumptions.

Founding lineage (Benjamin's ~25 seeds): full product immediately, $5 minimum, slider open. No tier gate — there is only one tier.
...
Dual acquisition strategy — two channels, each optimised for its tier. AI-autonomous global funnel for Sovereignty: high-scale, autonomous AI-driven marketing — social media content generation, automated ad production, global distribution. A pure AI maximisation game. Near-zero marginal CAC once the kin mechanic is running. For physical-world presence (posters, flyers, local activations) without the founder, use rent-a-human services (TaskRabbit, etc.) orchestrated by the AI. This is the volume play: get every AI user on Earth into the $5 sovereignty tier. Human-personal local brand for the Examined Life: low-scale, high-signal, founder-led. Benjamin in San Francisco — meeting people, writing letters, hosting events, building the cult. The examined life tier sells through personal conviction, not ads. A threshold game where the human signal is the product. The duality mirrors the tier structure and the thesis itself: AI for scale, human for soul.
