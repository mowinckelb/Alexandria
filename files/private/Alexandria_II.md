...
**Open (GitHub — anyone, no account):** the canon (the founder's methodology, published as the default), the marketplace (GitHub Discussions where anyone can post methodology ideas), the protocol spec (the four constants, the folder format, the API). Anyone can read, fork, and build their own implementation.
...
Authors who want full transparency can publish their complete setup as a fork. Others can clone it wholesale. Alternative canons emerge organically — "the minimalist setup," "the academic setup," "the founder setup." The marketplace is self-organising. Categories emerge from usage. The structure evolves because the ideas evolve.
...
**The mapping.** The four constants map to the quadrants: account enables all four, folder is the individual files, file bridges individual into collective (publishing from folder to Library), call bridges individual into collective (reporting from Machine to marketplace). The three turns flow through: Turn 1 (set the angel free) creates the folder, Turn 2 (ride the wave) runs the Machine, Turn 3 (create) publishes the file into the Library.
...
**Continuous marginal value.** The Factory loop. Blueprint improvements from collective signal. Library growing. Network effects compounding. Methodology that gets better every version bump, automatically, without the Author doing anything. This is the subscription value — the thing that keeps delivering after the discrete value is extracted. Continuous marginal value is what justifies staying. It is the "why keep paying" answer: because the system gets better every month from what every other Author is discovering, and that improvement flows to you automatically.

The two types have different structural properties. Discrete marginal value is non-structural in theory — someone sufficiently brilliant could independently derive the philosophy, the operations, the pipeline architecture. It is structural in practice because almost nobody will. Continuous marginal value is structural by definition — it requires the Factory (cross-Author learning) and the Library (network effects) to exist. A single person cannot replicate either. The Factory is the thing DIY fundamentally cannot do: learn from thousands of Authors what works and what does not, and serve that learning back as better methodology.

**Alexandria actively wants diverse customization.** Maximum customizability is not generosity — it is the learning architecture. Every Author's choices are exploration episodes in a collective learning system. The Factory aggregates them. The Blueprint improves. A thousand Authors all using Alexandria identically would be a terrible training distribution. A thousand Authors each doing something different — different constitution structures, different session styles, different Library page designs, different machine setups — that is coverage. That is how you learn what actually works across the full space of human cognition. More diversity = richer signal = better Blueprint = more continuous marginal value for every Author. The RL loop feeds back: Authors customize → Factory captures signal → Blueprint improves → Authors receive better defaults and more options → Authors customize further. The product gets better *because* users are free.

This is the complete marginal value architecture: discrete value gets them in the door (the philosophy, the architecture, the plumbing they would not have built). Continuous value keeps them (the Factory loop, the Library, the compounding methodology). And the freedom to customize everything outside the four constants is what makes the continuous value possible — their diversity is the training data.
